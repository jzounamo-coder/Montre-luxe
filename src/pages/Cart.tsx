    ...
    script.onload = () => {
      ...
      try {
        const handler = (window as any).PaystackPop.setup({
          ...
          // On utilise le GHS ou USD pour le test si le XAF bloque en mode starter
          amount: Math.round(total * 100), 
          currency: 'USD', 
          ref: 'PRESTIGE-' + Date.now(),
          callback: async (response: any) => {
            console.log("Paiement réussi Paystack", response);
            // On enregistre dans Supabase comme "payé" après le succès Paystack
            const success = await saveOrder('paystack', 'payé');
            if (success) {
              toast.success("Paiement validé ! Votre ticket a été ajouté.");
              setShowCheckoutModal(false);
              navigate('/profile');
            }
          },
        ...
        toast.error("Erreur d'initialisation du guichet.");
      }
    };
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-1">
            <div className="luxury-card p-8 sticky top-32">
              <h3 className="font-serif text-2xl mb-8 text-center lg:text-left">Récapitulatif</h3>
      ...
      <AnimatePresence>
        {showCheckoutModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white dark:bg-zinc-950 w-full max-w-md p-8 rounded-2xl shadow-2xl relative border border-white/5">
              ...
              <div className="space-y-4">
                <button onClick={handleWhatsApp} className="w-full flex items-center gap-4 p-4 border border-zinc-100 dark:border-white/5 hover:bg-zinc-50 dark:hover:bg-white/5 transition-all group rounded-xl text-left">
                  ...
                  <div><p className="font-serif text-lg">Option Prestige (WhatsApp)</p><p className="text-[10px] uppercase text-zinc-500">Paiement à la livraison</p></div>
                </button>

                <button onClick={handlePaystack} disabled={isLoadingPaystack} className="w-full flex items-center gap-4 p-4 border border-zinc-100 dark:border-white/5 hover:bg-zinc-50 dark:hover:bg-white/5 transition-all group rounded-xl text-left">
                  ...
                  <div><p className="font-serif text-lg">Paiement Sécurisé</p><p className="text-[10px] uppercase text-zinc-500">Mobile Money & Cartes</p></div>
                </button>
              </div>
            </motion.div>
          </div>
